...
getOps.on('completed', function(name, body, auth) {
  var response = getOps.display[name](body, prompt, auth);
  if(response){
    response(prompt, stack);
  } else if(response === false){
    console.log('\x1B[31mFeature not yet/completely implimented\x1B[39m');
  }
  counter=false;
  stack();
});
